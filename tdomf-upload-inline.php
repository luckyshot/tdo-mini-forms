...
if(!tdomf_can_current_user_see_form($form_id)) {
	tdomf_log_message("Someone with no permissions tried to access the inline-upload form!",TDOMF_LOG_BAD);
	unset($form_data['tdomf_upload_key_'.$form_id.'_'.$index]);
	$all_good = false;
	tdomf_save_form_data($form_id,$form_data);
...
if($all_good) {
	...
	// Only worry about uploaded files if the upload security key is good
	//
